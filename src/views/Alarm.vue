...
const form = ref({ title: '', time: '08:00', repeat: [], enabled: true })
...
const openAddModal = () => {
  ...
  form.value = { title: '', time: '08:00', repeat: [], enabled: true }
  showModal.value = true
}

const editAlarm = (alarm) => {
  editingAlarm.value = alarm
  form.value = { title: alarm.title, time: alarm.time, repeat: [...alarm.repeat], enabled: alarm.enabled }
  showModal.value = true
}
...
const saveAlarm = () => {
  ...
  if (editingAlarm.value) {
    ...
    if (index >= 0) alarms.value[index] = { ...alarms.value[index], ...form.value, title: form.value.title.trim() }
  } else {
    alarms.value.unshift({ id: Date.now().toString(), ...form.value, title: form.value.title.trim() })
  }
  persistAlarms()
  closeModal()
}
...
const toggleAlarm = () => persistAlarms()
...
const loadAlarms = () => {
  ...
  alarms.value = [
    { id: '1', title: '早安问候', time: '07:00', enabled: true, repeat: [1, 2, 3, 4, 5] },
    { id: '2', title: '晚安提醒', time: '22:00', enabled: true, repeat: [0, 1, 2, 3, 4, 5, 6] },
    { id: '3', title: '纪念日提醒', time: '09:00', enabled: false, repeat: [] }
  ]
  persistAlarms()
}
